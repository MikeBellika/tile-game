...
export default function Settings({
  ...
  return (
    <div>
      <AnimatePresence>
        {open && (
          <motion.dialog
            ...
            open={open}
            className="fixed top-[50%] z-50 h-min w-full translate-y-[-50%] rounded-lg border bg-black p-8 text-black sm:w-1/2  dark:text-white"
          >
            <h2 className="whitespace-nowrap text-lg font-semibold leading-none tracking-tight">
              Settings
            </h2>
            <span className="text-muted-foreground mt-2 text-sm">
              Control animation speed and game position.
            </span>
              ...
              {Object.keys(AnimationSpeeds).map((speed) => (
                <div key={speed}>
                  <input
                    ...
                    value={speed}
                    onChange={() => setAnimationSpeed(speed as AnimationSpeed)}
                  />
      ...
      <button
        className="py-4 text-gray-700 dark:text-gray-400"
